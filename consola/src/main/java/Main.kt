...
fun menuVehiculos(scanner: Scanner, gestorVehiculos: GestorVehiculos) {
        ...
        try {
            ...
            when (scanner.nextLine().toInt()) {
                ...
                3 -> {
                    ...
                    print("Nueva placa (deje vacío para no cambiar): ")
                    val placa = scanner.nextLine()

                    print("Nueva marca (deje vacío para no cambiar): ")
                    val marca = scanner.nextLine()

                    print("Nuevo modelo (deje vacío para no cambiar): ")
                    val modelo = scanner.nextLine()
...
fun main() {
    ...
    // 2.  Creamos la memoria del sistema UNA SOLA VEZ aquí
    ...
    while (!salir) {
        ...
        println("2. Gestión de Vehículos y Planes (Rodolfo Herrera)")
        ...
        try {
            when (scanner.nextLine().toInt()) {
                // 3.  Le pasamos la memoria intacta al menú
                1 -> menuClientes(scanner, gestorClientes)
                2 -> menuVehiculos(scanner, gestorVehiculos)
